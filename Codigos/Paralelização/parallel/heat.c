...
#pragma omp parallel for default(none) shared(u, N, outside_temp)
...
#pragma omp parallel for default(none) shared(f, box_first, box_last)          \
    collapse(2)
...
#pragma omp parallel for default(none) shared(newu, N, outside_temp)
  for (size_t i = 0; i < N + 2; ++i) {
...
#pragma omp parallel for default(none) shared(N, u, newu, h, f)                \
    reduction(max                                                              \
              : error) schedule(static) collapse(2)
