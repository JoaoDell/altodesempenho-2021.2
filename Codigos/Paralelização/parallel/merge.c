...
void mergesort_rec(int *start, int *finish) {
  ...
  if (N > PARALLEL_LIMIT) {
    // If lots of elements, sort in parallel.
    int *middle = start + N / 2;
    // Invariant: middle > start && finish > middle
#pragma omp task default(none) firstprivate(start, middle)
    mergesort_rec(start, middle);
#pragma omp task default(none) firstprivate(middle, finish)
...
#pragma omp taskwait
  ...
  if (N > 1) {
    // If few elements, sort sequentially.
    int *middle = start + N / 2;
    // Invariant: middle > start && finish > middle
    mergesort_rec(start, middle);
    mergesort_rec(middle, finish);
