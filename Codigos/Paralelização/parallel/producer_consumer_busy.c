...
#ifdef DEBUG
const long ACCREPORTSTEPS = 1000000;
#endif
...
int get_value(buffer_type *buffer) {
  while (true) {
    ...
    // Set a lock to assure exclusive access to the buffer.
    ...
    if (buffer->n > 0) {
      ...
      int res = buffer->data[buffer->n];
      // Release the lock before returning.
      omp_unset_lock(&buffer->lock);
      return res;
    }
    // Release the lock to give other threads a chance.
    omp_unset_lock(&buffer->lock);
  }
}
